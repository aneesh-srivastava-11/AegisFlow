...
export async function checkRateLimit(identifier, maxRequests = 10) {
  ...
  try {
    ...
    // Find existing rate limit doc
    const record = await collection.findOne({ _id: identifier });
    ...
    if (timestamps.length >= maxRequests) {
      // Save filtered timestamps to prevent growth
      await collection.updateOne(
        { _id: identifier },
        { 
          $set: { timestamps, updatedAt: new Date(now) },
