  ...
  "critical": [
    {
      ...
      "fix": "Human-readable description of what to change and why",
      "fix_code_snippet": "The exact replacement code (1-5 lines), ready to be applied via GitHub Suggested Changes. null if a simple code replacement is not applicable.",
...
11. Always try to populate fix_code_snippet when the fix is a simple line replacement`;
}
...
/**
 * Feature A: Build a prompt to generate a PR title, description, and changelog
 * from a code diff using Gemini.
 * @param {string} diff - The code diff to summarize
 * @param {string} language - Primary language detected
 * @param {string[]} files - List of changed files
 * @returns {string} Prompt string
 */
export function buildPRDescriptionPrompt(diff, language, files) {
  const fileList = files.slice(0, 20).join('\n');

  return `You are an expert software engineer writing a professional GitHub Pull Request description.
Analyze the following code diff and generate a concise, professional PR description.
...
{
  "title": "Short, imperative PR title (max 72 chars, e.g. 'Fix SQL injection in user auth endpoint')",
  "description": "Clear multi-paragraph Markdown description of what this PR does, why it was needed, and any important implementation notes. Use **bold** and bullet points where appropriate.",
  "changelog": "## What Changed\\n- Bullet-point list of specific changes suitable for a CHANGELOG.md entry",
  "breaking_changes": "null or a string describing any breaking API or behavior changes",
  "testing_notes": "How to test these changes manually or what tests were added"
}
