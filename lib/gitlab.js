 ...
 * Feature B: If fix_code_snippet is provided, renders it as a highlighted code block
 * for easy copy-paste (GitLab does not support one-click suggestions via the API yet,
 * so we use a clearly-labeled fenced code block).
 * @param {Object} issue 
 * @returns {string}
 */
function formatIssue(issue) {
  ...
  // Feature B: Render fix code as a named code block for easy copy-paste in GitLab
  if (issue.fix_code_snippet) {
    md += `**Suggested Fix:**\n\`\`\`suggestion\n${issue.fix_code_snippet}\n\`\`\`\n\n`;
    md += `**Explanation:** ${issue.fix}\n\n`;
  } else {
    md += `**Fix:** ${issue.fix}\n\n`;
  }
